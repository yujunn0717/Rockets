...
function openPositionPicker(anchor,payload){
  ...
  pop.innerHTML=`<div class="position-picker-title">${t('position')}</div><div class="position-picker-list">${positions.filter(p=>state.useDH||p!=='지명타자').map(p=>`<button type="button" data-position="${esc(p)}" class="${entry.position===p?'selected':''}">${posLabel(p,state.lang)}</button>`).join('')}</div>`;
...
function setupPositionTapAndHold(el,payload){
  let timer=null,startX=0,startY=0,pointerId=null,dragging=false;
  ...
  el.onpointerdown=e=>{
    ...
    timer=setTimeout(()=>{
      ...
      const g=$('#dragGhost');g.textContent=el.textContent.trim();g.classList.remove('hidden');
      moveGhost(e);
  ...
  el.onpointermove=e=>{
    ...
    if(!dragging&&dist>9){cancelTimer();el.classList.remove('pressing')}
    if(dragging&&drag&&drag.el===el){moveGhost(e);highlightTarget(e.clientX,e.clientY)}
  };
  el.onpointerup=e=>{
    if(pointerId!==e.pointerId)return;
    ...
    }else if(Math.hypot(e.clientX-startX,e.clientY-startY)<=9){
      openPositionPicker(el,payload);
    }
  };
  el.onpointercancel=()=>{cancelTimer();el.classList.remove('pressing','long-press-active');pointerId=null;dragging=false;if(drag&&drag.el===el)clearDrag()};
}
