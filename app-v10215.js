...
function renderPortrait(){const host=$('#portraitTable');host.innerHTML=`<div class="portrait-lineup"><section class="portrait-roster-section portrait-starters-watermark"><h3>${t('starters')}</h3><div class="portrait-head"><span>#</span><span>${t('number')}</span><span>${t('name')}</span><span>${t('position')}</span></div><div id="pStart"></div></section><div class="bench-divider"></div><section class="portrait-roster-section portrait-bench-watermark"><h3>${t('bench')}</h3><div class="portrait-head bench-only"><span>${t('number')}</span><span>${t('name')}</span></div><div id="pBench"></div></section></div>`;state.starters.forEach((e,i)=>{$('#pStart').insertAdjacentHTML('beforeend',portraitRow(e,i,'starter'))});state.bench.forEach(e=>{$('#pBench').insertAdjacentHTML('beforeend',benchPortraitRow(e))});$$('.name-drag').forEach(el=>setupDrag(el,{type:'order',id:el.dataset.id,source:el.dataset.source}));$$('.position-native-select').forEach(sel=>{sel.onchange=ev=>{const entry=state.starters.find(x=>x.id===sel.dataset.id);if(!entry)return;snapshot();entry.position=ev.target.value;commit()}});$$('.position-drag-handle').forEach(el=>setupPositionDragHandle(el,{type:'positionCell',id:el.dataset.id,source:el.dataset.source}));$$('[data-drop-row]').forEach(el=>el.dataset.dropRow=el.dataset.dropRow)}
function portraitRow(e,i,source){const p=player(e.playerId);const opts=positions.filter(pos=>state.usesDH||pos!=='지명타자').map(pos=>`<option value="${esc(pos)}" ${e.position===pos?'selected':''}>${posLabel(pos,state.lang)}</option>`).join('');return `<div class="portrait-row" data-drop-row="${e.id}" data-source="${source}"><span>${i+1}</span><span>${p?.num||'-'}</span><span class="drag-cell name-drag" data-id="${e.id}" data-source="${source}">${esc(p?.name||'—')}</span><span class="position-cell-control"><select class="position-native-select" data-id="${e.id}" aria-label="${t('position')}">${opts}</select><button type="button" class="position-drag-handle" data-id="${e.id}" data-source="${source}" aria-label="포지션 드래그">≡</button></span></div>`}
...
function setupPositionDragHandle(el,payload){
  let timer=null,pointerId=null,startX=0,startY=0,active=false;
  const stopScroll=()=>{document.documentElement.classList.add('drag-scroll-lock');document.body.classList.add('drag-scroll-lock')};
  const restoreScroll=()=>{document.documentElement.classList.remove('drag-scroll-lock');document.body.classList.remove('drag-scroll-lock')};
  const cleanup=()=>{if(timer){clearTimeout(timer);timer=null}el.classList.remove('pressing','long-press-active');pointerId=null;active=false;restoreScroll()};
  ...
  el.onpointerdown=e=>{
    ...
    el.classList.add('pressing');
    timer=setTimeout(()=>{
      timer=null;active=true;stopScroll();
      el.setPointerCapture?.(pointerId);
      drag={payload,startX,startY,el};
  ...
  el.onpointermove=e=>{
    if(pointerId!==e.pointerId)return;
    const dist=Math.hypot(e.clientX-startX,e.clientY-startY);
    if(!active&&dist>8){if(timer){clearTimeout(timer);timer=null}el.classList.remove('pressing')}
    if(active&&drag&&drag.el===el){e.preventDefault();e.stopPropagation();moveGhost(e);highlightTarget(e.clientX,e.clientY)}
  };
  el.onpointerup=e=>{
    if(pointerId!==e.pointerId)return;
    const wasActive=active;
    if(timer){clearTimeout(timer);timer=null}
    if(wasActive){e.preventDefault();e.stopPropagation();const target=findTarget(e.clientX,e.clientY);clearDrag();if(target)handleDrop(payload,target)}
    cleanup();
  };
  el.onpointercancel=()=>{if(drag&&drag.el===el)clearDrag();cleanup()};
}
...
function drawLineupTable(ctx,top,width=1200,watermarkImg=null){
  ...
  const orderX=left;
  const numberX=left+90;
  const nameStart=left+230;
  const contentWidth=right-nameStart;
