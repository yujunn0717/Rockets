...
function attendanceDefaultIds(g){
  return (Array.isArray(g.players)?g.players:state.players).map(p=>p.id).filter(Boolean);
}
...
function renderSettings(){
  ...
  if(!managed.length){
    ...
    am.innerHTML=managed.map(({g,i,group})=>{
      ...
      const savedPlayers=[...(Array.isArray(g.players)?g.players:state.players)].sort((a,b)=>jerseyNumberValue(a.num)-jerseyNumberValue(b.num));
