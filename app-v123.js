...
function setTitleEditing(active){
  topbarEl?.classList.toggle('title-editing',active);
  ['#undoBtn','#redoBtn','#saveGameBtn','#shareBtn'].forEach(sel=>{
    const el=$(sel);
    if(!el)return;
    if(active){
...
function setOfflineStatus(text, stateClass='pending'){
  const el=document.getElementById('offlineStatus');
  if(!el)return;
  el.textContent=text;
  el.classList.remove('ready','pending','error');
  el.classList.add(stateClass);
}

async function verifyOfflineCache(){
  try{
    const cache=await caches.open('rockets-lineup-manager-v1.2.2');
    const required=['./index.html','./styles-v122.css','./app-v122.js','./baseball-field.png','./rockets-watermark.png'];
    const checks=await Promise.all(required.map(async path=>!!(await cache.match(path))));
...
async function initOfflineSupport(){
  if(!('serviceWorker' in navigator) || !('caches' in window)){
    setOfflineStatus('이 브라우저는 오프라인 실행을 지원하지 않아요','error');
    return;
  }

  setOfflineStatus('오프라인 사용 준비 중…','pending');

  try{
    const reg=await navigator.serviceWorker.register('./sw.js');

    const markWhenReady=async()=>{
      try{
        await navigator.serviceWorker.ready;
        let ok=await verifyOfflineCache();
        if(ok){
          setOfflineStatus('오프라인 사용 가능','ready');
          return;
        }

        // Cache installation can finish just after SW ready on some browsers.
        for(let i=0;i<10;i++){
          await new Promise(r=>setTimeout(r,300));
          ok=await verifyOfflineCache();
          if(ok){
            setOfflineStatus('오프라인 사용 가능','ready');
            return;
          }
        }
        setOfflineStatus('오프라인 준비 중… 잠시 후 다시 확인해 주세요','pending');
      }catch(err){
        console.error(err);
        setOfflineStatus('오프라인 준비 상태를 확인할 수 없어요','error');
      }
    };
    ...
    navigator.serviceWorker.addEventListener('controllerchange',()=>markWhenReady());

    window.addEventListener('online',()=>markWhenReady());
    window.addEventListener('offline',async()=>{
      const ok=await verifyOfflineCache();
      setOfflineStatus(ok?'오프라인 사용 가능':'오프라인 캐시가 아직 준비되지 않았어요',ok?'ready':'error');
    });
  }catch(err){
    console.error(err);
    setOfflineStatus('오프라인 준비에 실패했어요','error');
  }
}

window.addEventListener('load',initOfflineSupport);
...
if('serviceWorker' in navigator){
  navigator.serviceWorker.addEventListener('message',async event=>{
    if(event.data?.type==='OFFLINE_CACHE_READY'){
      const ok=await verifyOfflineCache();
      if(ok)setOfflineStatus('오프라인 사용 가능','ready');
    }
  });
}
