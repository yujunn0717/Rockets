...
$$('#languageGate button').forEach(b=>b.onclick=()=>{state.lang=b.dataset.lang;$('#languageGate').classList.add('hidden');commit()});if(!state.lang)$('#languageGate').classList.remove('hidden');if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js'));sortPlayersByNumber();
renderAll();
